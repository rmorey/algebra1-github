'''
This is subtraction with variables. This problem set includes 3 types of problems 
j*x - k*x = l*x
j - k*x = k*x - j
j*x - k = j*x - k
...
def calculate_difference_of_terms(first_coefficient,first_variable,second_coefficient,second_variable):
    ...
    distractors = [first_coefficient - second_coefficient, first_coefficient*x - second_coefficient*x, -difference, first_term + second_term, first_coefficient*second_variable - second_coefficient*first_variable]
    return [difference, distractors, first_term, second_term]
...
def wrapper():
    for first_coefficient in range(-10,11):
	    ...
	    [answer,distractors,first_term,second_term]= calculate_difference_of_terms(first_coefficient,first_variable,second_coefficient,second_variable)
	    if answer:
	       answer_list= make_answer_list(answer,distractors,3)
	       if answer_list and random.choice([0,1]):
				[question_string,answer_list_strings]=format_difference_of_terms(first_term,second_term,answer_list)
				insert_html_html_multiple_question("Calculate the difference",question_string,answer_list_strings,myFile)
    myFile.close()

def format_difference_of_terms(first_term,second_term,answer_list):
